...
if(isset($_POST["btn"])){
    ...
    //câu lệnh insert SQL
    $sql= "INSERT INTO users ( usersname,password,email,image,role)
    VALUES('$usersname', '$pass','$email', '$image','$role')";
    
    //CHUẨN BỊ
    $stmt= $conn->prepare($sql);
    //nếu thực thi thành công 
    if($stmt->execute()){
        //upload images lên sever
        if(!empty($image)){
...
?> 
                  <label for="">usersname <span>*</span></label>
                   <input type="text" placeholder="usersname " name="usersname">
 <?php
if(isset($pass_err)){
    echo $pass_err;
}
              ...
              <div class="col-md-6">
                <div class="aa-myaccount-register">                 
                 ...
                 <form action="" class="aa-login-form" method="post"  enctype="multipart/form-data">
                    ...
                    <input type="file" placeholder="Email " name="image">
                    <label for="">Role <span>*</span></label><br>
                    <input type="number" placeholder="role " name="role">
                    <br>
                    <button type="submit" class="aa-browse-btn" name="btn">Register</button>                    
                  </form>
                </div>
              </div>
            </div>          
